...
| D6 | Line vocabulary | 15 line types (section 6.1). `wipe` and `grading` are one line `wipe_grading`. `holding_cost` is the only estimate line (`is_estimate = true`). Write-downs stay a `main` management view and are never a ledger line. |
