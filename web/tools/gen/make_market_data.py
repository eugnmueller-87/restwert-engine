...
SERIES_MIN, SERIES_GRADE = 3, "B"   # the series table: grade B asks, a series needs this many to show
b = a[a["grade"] == SERIES_GRADE]
...
ser = ser[ser["qty"] >= SERIES_MIN].sort_values(["family", "monate"])
...
data = {"today": TODAY, "example": example, "anchors": anchors, "curves": curves, "series": series, "studies": studies,
        "n_models": int(a["slug"].nunique()), "n_anchors": int(len(a)), "n_oem": int(a["oem"].nunique()), "series_min": SERIES_MIN, "series_grade": SERIES_GRADE}
