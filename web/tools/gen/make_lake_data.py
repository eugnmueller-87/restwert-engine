...
FEEDS = {  # feed key: (German name, delivering system in German, content in one line, needed for the cycle, public reference)
    ...
    "market/curves": ("Marktkurven", "Reiter Realisierung (öffentliche Preisbelege)", "Kurven je Familie und Hersteller aus den öffentlichen Preisbelegen; nur Vergleich, keine Entscheidung", False, True),
...
for _, r in miss.iterrows():
    for key in str(r["missing_steps"]).split(","):
        ...
        if key == "credited_at":
            ...
            hint = "kein Versandprotokoll mit dieser Seriennummer; im Kanal Versandprotokoll liegen " + str(unresolved_ship) + " ungeklärte Zeilen mit unbekannter Seriennummer; ob eine davon hierher gehört, klärt die Quelle (zu prüfen)"
        else:
