...
| `tools/gen/studies_de.json`, `tools/gen/faq.json`, `tools/gen/kpi_rahmen.json` | curated knowledge with URLs: the published studies (international, 13.09.2026; German sources, 16.09.2026) and the FAQ entries; `kpi_rahmen.json` is the KPI frame (Fassung 4, 18.09.2026, sets A to D) with, per KPI, a machine-readable target and the mapping to the engine (`engine.computable` ja, teilweise, nein with source and note). FAQ answers carry no number of the tool as text, only placeholders `{fact, fmt}` that `make_faq_data.py` fills from the run | hand, from read-only research agents |
...
| Analytics | Laufzeit | `term.js` | one term against a comparison term on a chosen device (since 17.09.2026, the owner's question "why does it start with iPhone after 12 months?"): device from the catalogue (family, manufacturer, model; default the series with the most evidence and its youngest generation, from the data), term and comparison term as pill switches; tiles, curve and "Was man daraus macht" follow the choice. The curve is the series' (family x manufacturer, the finest level with an age curve), the costs to sale come from the simulation's four buckets; outside the evidence span a number stands in brackets, without a usable curve "nicht belegt" | `outputs/market_curves.csv`, `outputs/market_anchors.csv`, `data/catalogue/*.csv`, `silver.device_ledger`, `config/lake.yaml` |
| Analytics | KPIs | `kpis.js` | the KPI tracker (rebuilt 17.09.2026 on the owner's request: no explanations, everything straight down): per KPI of the frame (Fassung 4 since 18.09.2026: sets A to C plus Satz D, ESG, D1 to D6) one row with Ziel, Ist, Fortschritt (bar and percent, share of the way from the baseline to the target, plus "Soll heute"), Bis (the cycle segment the target falls in, switchable Quartal, Halbjahr, Jahr), Beiträge per role, Eigner (Team, or Individuell with one of the six roles and a name; kept in `localStorage` only, filters the Beiträge to that role, never exported), Status never guessed; a row unfolds its contribution lines (Datum, Rolle, Maßnahme, Wirkung, Beleg-ID, Stufe) and the link "Definition" to the frame | `outputs/gold__kpi_values.csv`, `outputs/kpi_values.csv`, `config/kpi_targets.yaml`, `config/performance_cycle.yaml` (cycle and start date), `config/owners.yaml` (the six roles and what they own), `config/esg.yaml` (CO2e avoidance factor per family with source, for D2), `silver.device_ledger`, `silver.contracts`, `main.indirect_spend`, `main.rv_forecast_error_monthly`, `main.device_pnl`, `main.decision_log`, `bronze.erp_supplier_invoices`, `bronze.cat_models`, `bronze.sd_tickets` (D3), `bronze.rf_work_orders` (contributions D1, D2), `data/report.json`; the frame itself from `tools/gen/kpi_rahmen.json` |
